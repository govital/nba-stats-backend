...
public class GameStatController {
    ...
    public ResponseEntity<?> getPlayerSeasonAverage(@PathVariable int playerId) {
        Map<String, Double> averages = gameStatService.getPlayerSeasonAverage(playerId);
        if (averages.isEmpty()) {
            return ResponseEntity.status(HttpStatus.NOT_FOUND).body(Map.of("error", "No season stats found for player"));
        }
        return ResponseEntity.ok(averages);
    }

    @GetMapping("/team/{teamId}/average")
    public ResponseEntity<?> getTeamSeasonAverage(@PathVariable int teamId) {
        Map<String, Double> averages = gameStatService.getTeamSeasonAverage(teamId);
        if (averages.isEmpty()) {
            return ResponseEntity.status(HttpStatus.NOT_FOUND).body(Map.of("error", "No season stats found for team"));
        }
        return ResponseEntity.ok(averages);
    }
}
